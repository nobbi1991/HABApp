...
class _CoordinateIndex(IntEnum):
    LATITUDE = 0
    LONGITUDE = 1
    ELEVATION = 2
...
class Point:
    ...
    def distance(
        self,
        other: Point | tuple[float, float, float | None] | tuple[float, float],
        *,
        unit: Literal['km', 'kilometers', 'meters', 'm', 'miles', 'mi', 'feet', 'fi'] = 'kilometers',
    ) -> float:
    ...
    def __getitem__(self, item: int | str) -> float:
        if isinstance(item, int):
            if item == _CoordinateIndex.LATITUDE:
                return self._latitude
            if item == _CoordinateIndex.LONGITUDE:
                return self._longitude
            if item == _CoordinateIndex.ELEVATION:
                return self._elevation
            raise IndexError()
