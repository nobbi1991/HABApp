...
_MAX_RUN_DURATION = 0.8
_MAX_START_DURATION = 0.05
...
class PoolFunc(ContextProvidingObj):
    ...
    def run(self) -> Any:
        ...
        try:
            ...
            if self.dur_start > _MAX_START_DURATION:
                parent.log.warning(
                    f'Starting of {parent.name} took too long: {self.dur_start:.2f}s. '
            ...
            if parent.warn_too_long and self.dur_run > _MAX_RUN_DURATION and self.usage_high >= POOL_THREADS * 0.6:
                parent.log.warning(
                    f'{self.usage_high}/{POOL_THREADS} threads have been in use and '
